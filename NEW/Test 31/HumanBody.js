...
class HumanBody{
  ...
  return sign * y; // erf(-x) = -erf(x);
    ...
    run_simulation(){
        while(true){
            var val;
            while( (val = this.fire_event()) == 1){this.processTick();
                this.ticks++;
    ...
    readExerciseFile(file){
        ...
        for(var line = 0; line < lines.length; line++){
           ...
           console.log(this.exerciseArray);
            ...
            console.log("exercise: " + this.exerciseArray[lineNum].name + " " + this.exerciseArray[lineNum].intensity);
            lineNum++;
    ...
    fire_event(){
        ...
        var event_type = event_.event_type;
